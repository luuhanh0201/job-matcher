...
export class AiExtractorService {
  ...
  async extractCv(cvText: string): Promise<ExtractedCvData> {
    ...
    for (let attempt = 1; attempt <= MAX_RETRIES; attempt += 1) {
      try {
        ...
        lastError = error;
        this.logger.warn(
          `Lần thử trích xuất CV ${attempt}/${MAX_RETRIES} thất bại: ${
    ...
    this.logger.error(
      'Trích xuất CV thất bại sau số lần thử tối đa',
      lastError as Error,
    );
  ...
  private loadSystemPrompt(): string {
    try {
      ...
      this.logger.warn(
        `Không thể đọc file promt-AI-extractor.md, sử dụng fallback prompt: ${
          error instanceof Error ? error.message : 'lỗi không xác định'
        }`,
      );
