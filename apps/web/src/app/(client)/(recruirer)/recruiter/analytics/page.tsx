...
export default function RecruiterAnalyticsPage() {
  return (
    <div className="space-y-5">
      <header>
        <h1 className="text-2xl font-black text-foreground">AI Candidate Finder</h1>
        <p className="mt-1 text-sm text-muted-foreground">
          Phân tích dữ liệu ứng viên, gợi ý nguồn tuyển phù hợp và dự báo hiệu suất chiến dịch.
        </p>
      </header>

      <section className="rounded-2xl border border-border bg-linear-to-r from-primary/10 to-card p-5 shadow-sm">
        <div className="inline-flex items-center gap-2 rounded-full bg-card px-3 py-1 text-xs font-bold text-accent">
      ...
      <section className="grid grid-cols-1 gap-3 md:grid-cols-3">
        {insights.map(({ title, value, desc, icon: Icon }) => (
          <article key={title} className="rounded-2xl border border-border bg-card p-4 shadow-sm">
            ...
            <p className="mt-3 text-xs font-semibold uppercase tracking-wide text-muted-foreground">{title}</p>
      ...
      <section className="rounded-2xl border border-dashed border-border bg-card p-5">
        <p className="text-sm font-semibold text-foreground">Biểu đồ hiệu suất (placeholder)</p>
        <div className="mt-4 grid h-44 grid-cols-7 items-end gap-2">
          {[45, 60, 52, 80, 68, 74, 90].map((height, index) => (
            <div key={index} className="rounded-md bg-primary/80" style={{ height: `${height}%` }} />
          ))}
        </div>
      </section>
    </div>
