...
export default function RecruiterCandidatesPage() {
  return (
    <div className="space-y-5">
      <header>
        <h1 className="text-2xl font-black text-foreground">Danh sách ứng viên</h1>
        <p className="mt-1 text-sm text-muted-foreground">Kho ứng viên đã lưu và ứng viên được AI đề xuất cho công ty bạn.</p>
      </header>

      <section className="relative rounded-2xl border border-border bg-card p-3 shadow-sm">
        ...
        <input
          ...
          className="h-11 w-full rounded-xl bg-muted pl-10 pr-3 text-sm outline-none focus:ring-2 focus:ring-primary/30"
        />
      </section>

      <section className="grid grid-cols-1 gap-3">
        {candidates.map((candidate) => (
          <article key={candidate.email} className="rounded-2xl border border-border bg-card p-4 shadow-sm">
            ...
            <div className="mt-3 flex flex-wrap gap-2">
              <button className="inline-flex items-center gap-1 rounded-lg bg-primary/10 px-3 py-1.5 text-xs font-bold text-primary hover:brightness-95">
